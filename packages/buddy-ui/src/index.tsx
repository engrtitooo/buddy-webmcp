...
import { CapabilityMapper, MockAgentProvider, PermissionEngine, RepeatedToolCallGuard, normalizeAgentDecision, type AgentProvider } from '@buddy/agent-core';
...
type VoiceSubmissionMode = 'review' | 'auto';
interface ChatMessage { id: string; role: 'user' | 'assistant'; text: string }
interface AvatarPosition { x: number; y: number }
interface StoredSettings { locale: Locale | 'auto'; muted: boolean; theme: 'light' | 'dark' | 'system'; developerMode: boolean; voiceSubmissionMode: VoiceSubmissionMode; rules: AgentRules; position?: AvatarPosition }
...
async function loadSettings(): Promise<StoredSettings> {
  ...
  if (typeof chrome !== 'undefined' && chrome.storage?.local) {
    const result = await chrome.storage.local.get('buddySettings'); stored = result.buddySettings as Partial<StoredSettings> | undefined ?? {};
  } else {
    try { stored = JSON.parse(localStorage.getItem('buddySettings') ?? '{}') as Partial<StoredSettings>; } catch { stored = {}; }
  }
...
function ApprovalCard({ approval, locale, onApprove, onCancel }: { approval: PendingApproval; locale: Locale; onApprove: () => void; onCancel: () => void }) {
  ...
  return <section className="buddy-approval" aria-labelledby="buddy-approval-title">
    ...
    <dl><div><dt>What</dt><dd>{approval.what}</dd></div><div><dt>Why</dt><dd>{approval.why}</dd></div><div><dt>Site</dt><dd>{approval.site}</dd></div><div><dt>Risk</dt><dd>{approval.risk.replaceAll('_', ' ')}</dd></div><div><dt>Arguments</dt><dd><pre className="buddy-approval-arguments"><code>{approval.argumentsJson}</code></pre></dd></div></dl>
...
export function BuddyApp({ adapter, provider = new MockAgentProvider(), siteName = location.hostname, demoOpen = false }: { adapter: WebMCPAdapter; provider?: AgentProvider; siteName?: string; demoOpen?: boolean }) {
  ...
  useEffect(() => { void loadSettings().then((loaded) => { setSettings(loaded); if (loaded.position) setPosition(clampPosition(loaded.position)); setSettingsLoaded(true); }); }, []);
  useEffect(() => { if (settingsLoaded) void saveSettings({ ...settings, ...(position ? { position } : {}) }); }, [position, settings, settingsLoaded]);
  ...
  const continueRun = async (run: RunContext) => {
    try {
      while (run.nextTurn < AGENT_LIMITS.maxTurns && activeRun.current?.id === run.id && !run.controller.signal.aborted) {
        ...
        const decision = normalizeAgentDecision(raw, tools);

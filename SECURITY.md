...
- The public judging service explicitly enables `BUDDY_EXTENSION_ORIGIN_POLICY=chrome-extensions`: only complete `chrome-extension://[a-p]{32}` origins are accepted beyond the exact allowlist. Normal web origins remain restricted. Any installed extension or non-browser client could use this public demo, so this policy is not authentication. Text/voice quota buckets cannot be reset by rotating extension IDs. See [PRODUCTION.md](./PRODUCTION.md#extension-origin-security) for rationale, proxy quota limitations, and the private `allowlist` alternative

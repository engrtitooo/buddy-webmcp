...
- Server and client validate every selected tool name, and Ajv validates arguments against the site's current JSON Schema before execution; invalid calls become bounded repair feedback and never reach `executeTool`
...
- Approval shows complete human-readable values; raw JSON is Developer Mode-only, and Cancel never invokes the gated tool

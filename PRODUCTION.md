...
The opt-in `npm run smoke:production -- --expected-commit=<full-commit-sha>` performs those live checks, including preflights from two random valid extension IDs and rejection of arbitrary web/null/originless callers. It uses five paid synthetic text turns to check a valid site action, a direct capability explanation, a tool-result follow-up, and unrelated-question refusals both before and after a site result. It also performs one synthetic SDP bootstrap, with no tool execution, microphone, or media stream, and prints only safe status/UUID/commit/check metadata. It verifies bootstrap negotiation, not actual audio playback. It is excluded from unit tests and CI.
